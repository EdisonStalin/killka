...
class MsReportStock(models.TransientModel):
    ...
    def print_excel_report(self):
        ...
        columns = [
            (_('No'), 5, 'no', 'no'),
            (_('Product'), 30, 'char', 'char'),
            (_('Product Category'), 20, 'char', 'char'),
            (_('Location'), 30, 'char', 'char'),
            (_('Incoming Date'), 20, 'char', 'char'),
            (_('Stock Age'), 20, 'number', 'char'),
            (_('Total Stock'), 20, 'float', 'float'),
            (_('Available'), 20, 'float', 'float'),
            (_('Reserved'), 20, 'float', 'float'),
        ]
        ...
        worksheet.merge_range('A%s:B%s'%(row,row), _('Grand Total'), wbf['total_orange'])
        ...
        worksheet.write('A%s'%(row+2), '%s %s (%s)'%(_('Date'), datetime_string,self.env.user.tz or 'UTC'), wbf['content_datetime'])
        workbook.close()
