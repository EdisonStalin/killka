...
class VATReport(models.TransientModel):
    ...
    *** VATReportTax
    ...
    date_to = fields.Date()
    based_on = fields.Selection([('taxtags', 'Tax Tags'), ('taxgroups', 'Tax Groups')],string='Based On',
                                required=True, default='taxtags')
    tax_detail = fields.Boolean('Tax Detail')

    # Data fields, used to browse report data
    taxtags_ids = fields.One2many(comodel_name='report_vat_report_taxtag', inverse_name='report_id')
...
class VATReportTaxTags(models.TransientModel):
    ...
    report_id = fields.Many2one(comodel_name='report_vat_report', ondelete='cascade', index=True)
...
class VATReportCompute(models.TransientModel):
    ...
    def print_report(self, report_type='qweb'):
        self.ensure_one()
        if report_type == 'xlsx':
            report_name = 'oe_account_report.report_vat_report_xlsx'
        else:
            report_name = 'oe_account_report.report_vat_report_qweb'
    ...
    def _get_html(self):
        ...
        if report:
            rcontext['o'] = report
            result['html'] = self.env.ref('oe_account_report.report_vat_report').render(rcontext)
        return result
