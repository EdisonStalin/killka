...
class ReportJournalLedger(models.TransientModel):
    ...
    move_target = fields.Selection(selection=[('posted', 'All Posted Entries'),
                                    ('all', 'All Entries')], default='all', required=True,)
    ...
    def _get_sort_options(self):
        return self.env['account.print.journal']._get_sort_options()

    @api.model
    def _get_group_options(self):
        return self.env['account.print.journal']._get_group_options()
    ...
    def print_report(self, report_type):
        self.ensure_one()
        if report_type == 'xlsx':
            report_name = 'oe_account_report.report_journal_ledger_xlsx'
        else:
            report_name = 'oe_account_report.report_journal_ledger_qweb'
        return self.env['ir.actions.report'].search([('report_name', '=', report_name),
    ...
    def _get_html(self):
        ...
        if report:
            rcontext['o'] = report
            result['html'] = self.env.ref('oe_account_report.report_journal_ledger').render(rcontext)
        return result
