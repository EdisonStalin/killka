odoo.define('oe_web.web_export_view', function (require) {
    "use strict";
        ...
        _redraw: function () {
            ...
            if (self.getParent().renderer.viewType === 'list') {
                session.user_has_group(
                    'oe_web.group_disallow_export_view_data_excel')
            ...
            $.each(view.renderer.columns, function () {
                if (this.tag === 'field' && (this.attrs.widget === undefined || this.attrs.widget !== 'handle')) {
                    ...
                    var css_selector_index = isGrouped
                        ? column_index+2 : column_index+1;
                    column_header_selector = '.o_list_view > thead > tr> ' +
                        'th:not([class*="o_list_record_selector"]):eq(' +
                        css_selector_index + ')';
                    var header = view.$el.find(column_header_selector)[0].textContent;
                    export_columns_names.push(header);
                }
                ++column_index;
            });
            var export_rows = [];
