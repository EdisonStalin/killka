...
odoo.define('ks_pos_low_stock_alert.ks_low_stock', function (require) {
	...
	ks_super_pos.models.forEach(function(m) {
		if (m.model == 'product.product')
		{
			m.fields.push('type', 'qty_available');
			m.context = function(self){ return { display_default_code: false, location: self.config.stock_location_id[0]}; };
		}
	});
    ...
    ks_screens.PaymentScreenWidget.include({
        ...
        validate_order: function(force_validation) {
        	var order_valid = this.order_is_valid(force_validation);
        	var order_stock = ks_utils.ks_validate_order_items_availability(this.pos.get_order(), this.pos.config, this.gui)
            if (order_valid && order_stock) {
