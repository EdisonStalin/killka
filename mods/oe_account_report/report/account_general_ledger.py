...
class GeneralLedgerReport(models.TransientModel):
    ...
    **** GeneralLedgerReportMoveLine
    ...
    show_cost_center = fields.Boolean(default=lambda self: self.env.user.has_group('analytic.group_analytic_accounting'))

    # Data fields, used to browse report data
    account_ids = fields.One2many(comodel_name='report_general_ledger_account', inverse_name='report_id')

    # Compute of unaffected earnings account
    @api.depends('company_id')
    def _compute_unaffected_earnings_account(self):
        account_type = self.env.ref('account.data_account_type_current_assets')
        domain = [('user_type_id', '=', account_type.id), ('company_id', '=', self.company_id.id)]
        account_id = self.env['account.account'].search(domain, limit=1)
        self.unaffected_earnings_account = account_id
...
class GeneralLedgerReportAccount(models.TransientModel):
    ...
    # Data fields, used to browse report data
    move_line_ids = fields.One2many(comodel_name='report_general_ledger_move_line', inverse_name='report_account_id')
    partner_ids = fields.One2many(comodel_name='report_general_ledger_partner', inverse_name='report_account_id')
...
class GeneralLedgerReportPartner(models.TransientModel):
    ...
    # Data fields, used to browse report data
    move_line_ids = fields.One2many(comodel_name='report_general_ledger_move_line', inverse_name='report_partner_id')
...
class GeneralLedgerReportMoveLine(models.TransientModel):
    ...
    report_account_id = fields.Many2one(comodel_name='report_general_ledger_account', ondelete='cascade', index=True)
    report_partner_id = fields.Many2one(comodel_name='report_general_ledger_partner', ondelete='cascade', index=True)
...
class GeneralLedgerReportCompute(models.TransientModel):
    ...
    def print_report(self, report_type):
        self.ensure_one()
        if report_type == 'xlsx':
            report_name = 'oe_account_report.report_general_ledger_xlsx'
        else:
            report_name = 'oe_account_report.report_general_ledger_qweb'
        action = self.env['ir.actions.report'].search([('report_name', '=', report_name), ('report_type', '=', report_type)], limit=1)
    ...
    def _get_html(self):
        ...
        if report:
            rcontext['o'] = report
            result['html'] = self.env.ref('oe_account_report.report_general_ledger').render(rcontext)
        return result
    ...
    def compute_data_for_report(self,
        ...
        if (not self.filter_account_ids or self.unaffected_earnings_account.id in self.filter_account_ids.ids):
...
AND
    f.balance IS NOT NULL AND f.balance != 0
        ...
        query_inject_account_params += (self.id, self.env.uid,)
