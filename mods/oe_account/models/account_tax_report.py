...
class AccountTaxReportLine(models.Model):
    ...
    parent_id = fields.Many2one(string="Parent Line", comodel_name='account.tax.report.line', ondelete='cascade')
    ...
    report_id = fields.Many2one(string="Tax Report", required=True, comodel_name='account.tax.report.tax', ondelete='cascade', help="The parent tax report of this line")
    ...
    def create(self, vals):
        ...
        if tag_name and vals.get('report_id'):
            report = self.env['account.tax.report.tax'].browse(vals['report_id'])
