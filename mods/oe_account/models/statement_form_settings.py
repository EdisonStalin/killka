...
class StatementForm(models.Model):
    ...
    code = fields.Char(string='Statement Code', size=3, copy=True, required=True)
    ...
    action_id = fields.Many2one('ir.actions.act_window', string='Window Action', copy=True)
...
class AccountStatementTax(models.Model):
    _name = "account.statement.tax"
    _description = "Account Statement Tax"
    
    invoice_id = fields.Many2one('account.invoice', string='Invoice', ondelete='cascade', index=True)
    invoice_line_id = fields.Many2one('account.invoice.line', string='Invoice Line', ondelete='cascade', index=True)
    withhold_id = fields.Many2one('account.withholding', string='Withhold', ondelete='cascade', index=True)
    withhold_line_id = fields.Many2one('account.withholding.line', string='Withhold Line', ondelete='cascade', index=True)
    form_id = fields.Many2one('statement.form', string='Statement')
    name = fields.Char(string='Locker', size=5, required=True)
    tax_group_id = fields.Many2one('account.tax.group', string="Tax Group", required=True)
    base = fields.Monetary(string='Base', digits=dp.get_precision('Account Total'))
    percentage = fields.Percent(string='Percentage', required=True, default=0.0)
    amount = fields.Monetary(string='Amount', digits=dp.get_precision('Account Total'))
