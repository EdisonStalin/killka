...
class Form103(models.TransientModel):
    _name = 'form.103'
    _description = 'Form 103'
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'form_103_ir_attachments_rel', 'wizard_id', 'attachment_id', 'Attachments')


    @api.model
    def default_get(self, default_fields):
        res = super(Form103, self).default_get(default_fields)
    ...
    def generate_detail_xlxs(self):
        ...
        data['model'] = self.env.context.get('active_model', 'form.103')
        data['form'] = self.read(['date_start', 'date_end', 'company_id'])
        return self.env.ref('oe_account.form_103_xlsx').report_action(self, data=data)
    ...
    def generate_xml(self):
        ...
        form_103 = etree.Element('formulario')
        form_103.set('version', '0.2')
        ...
        form_103.append(header)
        details_103 = etree.Element('detalle')
        
        result_code = _get_details_form_103(self, data)
        ...
        for key in sorted(result_code):
            ...
            details_103.append(line)
        
        form_103.append(details_103)
        data_xml = etree.tostring(form_103, pretty_print=True, encoding='utf-8', xml_declaration=True, standalone=False)
        name = '%s-%s-%s' % ('103', self.date_start[0:4], self.date_end[5:7])
...
class Form103Xlsx(models.TransientModel):
    _name = 'report.oe_account.form_103_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        row += 1
        details_result = _get_details_form_103(self, data)
...
def _get_details_form_103(self, data):
