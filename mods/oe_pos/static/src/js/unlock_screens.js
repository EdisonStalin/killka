odoo.define('oe_pos.unlock_screens', function (require) {
"use strict";
...
var LoginScreenWidget = screens.ScreenWidget.extend({
    ...
    show: function(){
        ...
        if (! this.user.pos_security_pin){
            self.$('.pos_invalid_password').text(_t('Please set a security pin.'));
        }
        ...
        this.$('.login').click(function(){
            if (self.$('.user_password').val() === self.user.pos_security_pin){
                ...
                self.$('.pos_invalid_password').text(_t('Invalid Password'));
            }
        });
        $(document).keyup(function(e) {
        	if(!$($(document).find(".login-screen")[0]).hasClass('oe_hidden')){
	            if(e.keyCode == 13) {
	                e.preventDefault();
	                e.stopPropagation();
	                if (self.$('.user_password').val() === self.user.pos_security_pin){
