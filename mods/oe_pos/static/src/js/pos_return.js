odoo.define('oe_pos.return',function(require) {
"use strict";
...
models.load_models({
    model:  'pos.order',
    fields: ['name','invoice_id','partner_id','date_order','amount_total', 'amount_tax',
        'pos_reference','lines','state','session_id','company_id','return_ref','return_status'],
    domain: function(self) {return [['partner_id', '!=', self.config.default_partner_id[0]]]},
...
var ReturnWidget = PopupWidget.extend({
    ...
    render_list:function(options){
        ...
        rpc.query({
                ...
                for (var i = 1; i < tr.length; i++) {
                  ...
                  input.setAttribute("class", "refund_text");
                ...
                self.gui.show_popup('alert', {
                    title: _t('NO DATA'),
                    body: _t('Not this information of orders made in POS'),
                });
        		return;
            });
    },
    click_confirm: function(){
        ...
        if (count > 0){
            self.gui.show_popup('alert', {
                title: _t('Information entered'),
                body: _t('Please check the Returned Quantity,it is higher than purchased'),
            });
    		return;
        }
        else{
...
var OrderListScreenWidget = ScreenWidget.extend({
    ...
    search_order: function(query){
        ...
        for (var order_id in this.pos.orders){
            var r = re.exec(this.pos.orders[order_id]['pos_reference']+ '|'+ this.pos.orders[order_id]['invoice_id'][1]
            	+ '|'+ this.pos.orders[order_id]['return_ref'][1]+ '|'+ this.pos.orders[order_id]['partner_id'][1]);
            if(r){
...
models.PosModel = models.PosModel.extend({
    _save_to_server: function (orders, options) {
        ...
        var self = this;
        var timeout = typeof options.timeout === 'number' ? options.timeout : 8000 * orders.length;
